...
static int get_replica_exchange(FILE *fplog,const gmx_multisim_t *ms,
                                ...
                                int step,real time,real pres2)
{
    int  m,i,a,b;
    real *Epot=NULL,*Vol=NULL,*PresX2=NULL,*dvdl=NULL,*prob;
...
//            {
                ...
                dpV = (betaA*PresX2[a]-betaB*PresX2[b])*(Vol[b]-Vol[a])/16.6054;

       //         printf("%f\t%f\t%f\t%f\t%f\t%f\n",betaA,PresX2[a],betaB,PresX2[b],Vol[b],Vol[a]);
...
//                else
//                {
                prob[i] = exp(-delta)*(1.0-exp(-delta));
//                }
